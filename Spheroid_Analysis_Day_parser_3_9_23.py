...
for folder in folder_list:
    ...
    for image in pd.unique(df_last_day['ImageNumber']):
        ...
        # query_str = f'ImageNumber == {image} &ObjectNumber == {object} &  ((Location_Center_X < "{condition}" & group2 == "{control_condition}") | (group1 == "{control_condition}" & group2 == "{condition}"))'
            ...
            Location_x = df_last_day.loc[(df_last_day['ImageNumber'] == image) & (
                            df_last_day['ObjectNumber'] == object), 'Location_Center_X'].values[0]
            Location_y = df_last_day.loc[(df_last_day['ImageNumber'] == image) & (
                        df_last_day['ObjectNumber'] == object), 'Location_Center_Y'].values[0]

            df.loc[(df['ImageNumber'] == image) & (df['Location_Center_X'] < (Location_x + plus_minus_center)) & (
